...
```{r 03-009, fig.cap ="Diagrama del sistema de fiabilidad"}
# Diagrama
...
```{r 03-011, fig.cap ="Diagrama del sistema de metereología"}
# Diagrama
...
```{r 03-013, fig.cap ="Diagrama del sistema de inventario"}
# Diagrama
...
```{r 03-017, fig.cap ="Diagrama del sistema de planificación"}
# Diagrama
...
```{r 03-019, fig.cap ="Diagrama del sistema del Mercado de valores"}
# Diagrama
...
```{r 03-019-3, fig.cap ="Diagrama del sistema del problema de inventario."}
# Diagrama
...
g
```
y que nos permite extraer los ingresos netos totales esperados asumiendo que el periodo inicia con $i=5$ PCs en tienda, esto es, como $g(5,10)=$`r round(g[4],2)` euros.
...
Mientras que teóricamente obteníamos unos ingresos esperados de `r round(g[4],2)`€, con la simulación obtenemos una aproximación de `r round(beneficio[4],2)`€.
...
Así, en el idioma1, si partimos de un texto que empieza en vocal, esperamos encontrar `r round(mocupa1[1,1])` vocales y si partimos de una consonante, `r round(mocupa1[2,1],1)` vocales. En el idioma2, si el primer carácter es vocal, encontraré `r round(mocupa2[1,1],1)` vocales y `r round(mocupa1[2,1])` vocales si el texto empieza por consonante.
...
```
Tenemos así que el número medio de caracteres que esperamos encontrar en un texto hasta que aparezca por primera vez una vocal en el idioma 1 (partiendo de una consonante) es de `r round(mfpt1)`, y en el idioma2 de `r round(mfpt2)`.
...
Así, tenemos que en el idioma1 la probabilidad de que la primera consonante que encontremos esté tres caracteres después de la vocal de inicio es de `r fpm1[n]`, mientras que esta probabilidad en el idioma2 es sólo de `r fpm1[n]`.
...
Así, en el idioma1, desde la última consonante tendremos aproximadamente `r round(mrt1[2])` caracteres hasta encontrar otra consonante, mientras que en el idioma2 sólo `r round(mrt2[2])` por término medio.
