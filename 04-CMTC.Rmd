...
Podemos ver que el número de ciclos en que la máquina esta en fucionamiento es `r last(simulacion$nfun)` mientras que el número de veces que ha necesitado reparación son `r last(simulacion$nrep)`.
...
Por tanto, la proporción de tiempo que la máquina está en funcionamiento es `r round(tiempos[1]/365, 2)`, y el beneficio estimado para el próximo año viene dado por:
...
La proporción de tiempo en que la máquina está funcionando es del `r propor`% y el tiempo que está en reparación es del `r 100-propor`%.

El número de ocasiones en que la máquina está en reparación corresponde al número de veces que accedemos a la tarea de reparación, que en este caso es de `r nrow(reparacion)`.
...
Cualquier viaje inferior a `r limite` horas será un viaje seguro, mientras que si el tiempo es superior, la probabilidad de no tener un viaje seguro aumenta.
...
Cualquier viaje inferior a `r limite` horas será un viaje seguro, mientras que si el tiempo es superior, la probabilidad de no tener un viaje seguro aumenta. Podemos representar el gráfico de probabilidad de un viaje seguro:
...
La proporción de tiempo en que las cuatro máquinas están funcionando simultáneamente es `r round(100*res,2)`. Estudiamos la estabilidad del sistema realizando 500 simulaciones del sistema y estimando las cantidades de interés para la empresa.
...
Tenemos entonces que el número de máquinas en funcionamiento es `r mean(maquinasON, na.rm = TRUE)` y el tiempo de funcionamiento de las cuatro máquinas estimado es `r round(mean(propTimeON, na.rm = TRUE), 2)` para un periodo de 72 horas cuando al inicio todas las máquinas están paradas.
...
La probabilidad de interés es `r Pmat[1,5]` ($p_{15}(50)$) lo que demuestra que es factible que el sistema llegue al estado 4 partiendo del estado 0 después de 4 horas.
...
La probabilidad de interés viene dada por `r round(sum(ps[5:6]), 4)`, de forma que la máquina permancerá apagada sobre el 30% del tiempo.
...
El beneficio es `r beneficios[5, 1]` euros.
...
Por tanto, la probabilidad de un fallo que le impida al avión finalizar el viaje es `r 1-probabilidad`.
