...
::: {.yellowbox data-latex=""}
```{definition}
Dada una variable aleatoria $X$ con valor esperado $E(X)$ se define la **varianza** de $X$, $V(X)$ como:
...
```{r 01-005, fig.cap = "Función de masa de probabilidad y Función de distribución para el número de trabajadores de baja un día cualquiera."}
...
::: {.yellowbox data-latex=""}
```{definition, label = "vgeom"}
La variable aleatoria $N$ cuya función de masa de probabilidad viene dada en \@ref(eq:var-geometrica) se denomina variable Geométrica de parámetro $\theta$, y se denota por:
...
```{r 01-008, fig.cap = "Función de masa de probabilidad y Función de distribución para el día en que venderá el primer coche de lujo."}
# Valores de N
xs <- seq(0, 60, 1)
# Data frame
...
::: {.yellowbox data-latex=""}
```{definition, label = "vpoisson"}
La variable aleatoria $X$ cuya función de masa de probabilidad viene dada en \@ref(eq:var-poisson) se denomina variable poisson de parámetro $\lambda > 0$, y se denota por:
...
```{r 01-010, fig.cap = "Función de masa de probabilidad y Función de distribución para el número de barras de pan que se venden cada cinco minutos."}
# Valores de N
lambda=4
xs <- seq(0, 10, 1)
# Data frame
...
```{r 01-013,fig.cap="Probabilidad estimada de conseguir al menos 3 trozos de chocolate en cada galleta, en función de lambda."}
# Paso 1
set.seed(1970)
nsim <- 5000
...
Así pues, el número medio de trozos de chocolate que ha de suministrarse a cada galleta ha de ser al menos de `r lambda`.
...
::: {.yellowbox data-latex=""}
```{definition, label = "vuniforme"}
Una variable aleatoria $X$ tiene una distribución uniforme en el intervalo $[a, b]$, con $a,b \in R,
...
```{r 01-014, fig.cap="Función de densidad del tiempo de vida del marcapasos (en años)"}
lambda <- 1/16
...
La probabilidad de que el marcapasos dure más de 20 años y haya que reemplazarlo es de `r 1-round(pMC,3)`, por lo que efectivamente, es muy recomendable reemplazarlo antes. Sin embargo, al paciente en la pregunta (2) se le daría la misma recomendación, cuando la probabilidad de que el marcapasos dure más de 25 años desde su implante, que sería el tiempo que lo llevaría, es considerablemente inferior, `r 1-round(pexp(25,lambda),3)`. No es pues recomendable, utilizar esta distribución para modelizar el tiempo de vida de un implante.
...
```{r 01-016-2, fig.cap = "Tiempo de garantía recomendado en función de la probabilidad de que los motores necesiten reparación."}
# cuantil de interés
...
```{r graficosnormal,fig.cap="Funciones de densidad para varias distribuciones normales."}
x=seq(-10,10,0.1)
y1=dnorm(x)
y2=dnorm(x,0,3)
y3=dnorm(x,2,1)
...
```{r graficost,fig.cap="Funciones de densidad para varias distribuciones T de Student."}
x=seq(-5,5,0.1)
y1=dt(x,2)
y2=dt(x,5)
y3=dt(x,10)
...
```{r graficoschi2,fig.cap="Funciones de densidad para varias distribuciones Chi-cuadrado."}
x=seq(0,200,0.1)
y1=dchisq(x,5)
y2=dchisq(x,10)
y3=dchisq(x,50)
...
```{r graficosf,fig.cap="Funciones de densidad para varias distribuciones F-Snedecor."}
x=seq(0,5,0.01)
y1=df(x,5,5)
y2=df(x,1,5)
y3=df(x,50,10)
...
Interesa simular el funcionamiento del sistema en los próximos seis meses, de modo que dicho periodo expresado en minutos es de `r 6*30*24*60`, y la simulación parará cuando $ttotal<259200$ minutos, siendo
