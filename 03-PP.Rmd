...
Así, el número medio de pasajeros por coche será `r mu` y `r m2` su momento de orden 2. El número (total) esperado de pasajeros en 8 horas lo calcularemos multiplicando por la tasa de llegadas de coches, $\lambda$:
...
Por lo tanto, esperamos que lleguen a la estación en 8 horas alrededor de `r ey` pasajeros, con una desviación típica de `r round(sqrt(vy),2)`.
...
franja=c()
for(i in 1:nsim)
franja[i] = get_n_generated(simula.pp.simmer(tmax,lambda),"operacion")-1
```
